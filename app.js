(()=>{
"use strict";
const STORAGE_KEY="budget-familial-laetitia-final-v1",BACKUP_KEY=STORAGE_KEY+"-backup",DATA_VERSION=6;
...
const initial={
 ...
 goals:[{id:"g1",name:"Imprévus",target:1000,current:0}]
};
...
function migrate(saved){
 ...
 d.goals=Array.isArray(d.goals)?d.goals:clone(initial.goals);
...
function render(){
 ...
 renderSelects();renderAccounts();renderForecast();renderChart();renderGoals();renderCharges();renderHistory();renderSettings();
}
...
$("exportData").addEventListener("click",()=>{const blob=new Blob([JSON.stringify({app:"Budget Familial Laetitia",version:"6.0.0",data},null,2)],{type:"application/json"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="budget_laetitia_v6.json";a.click()});
