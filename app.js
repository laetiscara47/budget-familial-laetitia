...
const initial={
 ...
 expenses:[],incomeTransactions:[],transfers:[],deferredCardBatches:[],goals:[{id:"g1",name:"Imprévus",target:1000,current:0}],archives:[],settings:{darkMode:false,activeProfileId:"p1"},profiles:[{id:"p1",name:"Laetitia"}],vaultDocuments:[],loans:[],recurringIncomes:[]
};
function migrate(saved){
 ...
 d.transfers=Array.isArray(d.transfers)?d.transfers:[];d.goals=(Array.isArray(d.goals)?d.goals:clone(initial.goals)).map(g=>({...g,targetDate:g.targetDate||""}));d.archives=Array.isArray(d.archives)?d.archives:[];
 d.budgets=d.budgets&&typeof d.budgets==="object"?d.budgets:clone(initial.budgets);d.rules=Array.isArray(d.rules)?d.rules:clone(initial.rules);d.profiles=Array.isArray(d.profiles)&&d.profiles.length?d.profiles:[{id:"p1",name:"Laetitia"}];d.vaultDocuments=Array.isArray(d.vaultDocuments)?d.vaultDocuments:[];d.loans=Array.isArray(d.loans)?d.loans:[];d.recurringIncomes=Array.isArray(d.recurringIncomes)?d.recurringIncomes:[];d.settings={darkMode:Boolean(d.settings?.darkMode),activeProfileId:d.settings?.activeProfileId||d.profiles[0].id,showBriefing:d.settings?.showBriefing!==false,showEnvelopes:d.settings?.showEnvelopes!==false,showYearForecast:d.settings?.showYearForecast!==false,notifyCharges:d.settings?.notifyCharges!==false,notifyDeferredCard:d.settings?.notifyDeferredCard!==false,notifyBudgets:d.settings?.notifyBudgets!==false,notifyLowBalance:d.settings?.notifyLowBalance!==false,notificationDays:Number(d.settings?.notificationDays||3),autoGenerateIncome:d.settings?.autoGenerateIncome!==false,showLateIncome:d.settings?.showLateIncome!==false,detectAnomalies:d.settings?.detectAnomalies!==false,anomalyTolerance:Number(d.settings?.anomalyTolerance||20)};d.dataVersion=DATA_VERSION;delete d.bankBalance;return d;
}
...
let calendarDate=new Date(),selectedCalendarDate=today(),importRows=[],agendaPeriod="month",forecastHorizon=12;
...
function render(){
 ...
 ensureRecurringIncomeInstances();renderSelects();renderProfiles();renderRecurringIncomeSelects();renderAccountsV6();renderHeroV6();renderInfinitySerenity();renderIncomeEngine();renderRecurringIncomeSettings();renderSmartAgenda();renderGoalsV6();renderUltimateSnapshot();renderPatrimony();renderNotificationCenter();renderPremiumDecision();renderAccountComparison();renderMonthOverview();renderReportsSummary();renderV300Dashboard();renderInfinityFamilyTotals();renderV100Dashboard();renderDailyBriefing();renderProAlerts();renderEnvelopes();renderAlerts();renderSmartSummary();renderV30Insights();renderSubscriptions();renderSubscriptionSavings();renderMerchantAnalytics();renderForecast();renderChart();renderMonthlyStats();renderGoals();renderCalendar();renderHistory();renderAnomalies();renderLoans();renderVault();renderSettings();renderEngineSettings();renderProfileSettings();renderCategorySettings();renderRuleSettings();renderDataQuality();applyTheme();applyDashboardPreferences();
}
function renderSelects(){
 ...
 $("historyAccount").innerHTML=`<option value="">Tous les comptes</option>${opts}`;$("historyCategory").innerHTML=`<option value="">Toutes les catégories</option>${Object.keys(data.budgets).map(x=>`<option>${esc(x)}</option>`).join("")}`;$("expenseCategory").innerHTML=Object.keys(data.budgets).map(x=>`<option>${esc(x)}</option>`).join("");$("purchaseSimulationCategory").innerHTML=Object.keys(data.budgets).map(x=>`<option>${esc(x)}</option>`).join("");
}
...
function renderLoans(){
 $("loanCapitalTotal").textContent=euro(totalDebt());
 $("loanPaymentsTotal").textContent=euro(monthlyDebtPayments());
 $("loanAnnualCost").textContent=euro(monthlyDebtPayments()*12);
...
function upcomingTodayEvents(){
 const now=new Date(),day=now.getDate(),date=today();
 return [
  ...data.fixedCharges.filter(x=>!x.paid&&(x.day||28)===day).map(x=>({type:"charge",label:x.label,amount:-x.amount})),
  ...data.incomeTransactions.filter(x=>x.future&&x.date===date).map(x=>({type:"income",label:x.label,amount:x.amount})),
  ...data.expenses.filter(x=>x.date===date).map(x=>({type:"expense",label:x.label,amount:-x.amount}))
 ];
}
function renderDailyBriefing(){
 const events=upcomingTodayEvents();
 const daily=Math.max(0,totals().available/daysLeftInMonth());
 const projected=endMonth();
 $("briefingDate").textContent=new Date().toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long"});
 let text="";
 if(events.length){
  const outgoing=events.filter(e=>e.amount<0).reduce((s,e)=>s-Math.abs(e.amount),0);
  const incoming=events.filter(e=>e.amount>0).reduce((s,e)=>s+e.amount,0);
  text=`Aujourd’hui, ${events.length} opération${events.length>1?"s sont prévues":" est prévue"}.`;
  if(incoming)text+=` Entrées : ${euro(incoming)}.`;
  if(outgoing)text+=` Sorties : ${euro(Math.abs(outgoing))}.`;
 }else{
  text=`Aucune opération prévue aujourd’hui. Tu peux viser environ ${euro(daily)} de dépenses sans dépasser le disponible mensuel.`;
 }
 if(projected<300)text+=` Attention : la marge de fin de mois est limitée à ${euro(projected)}.`;
 $("dailyBriefingText").textContent=text;
 const chips=[];
 if(pendingDeferred()>0)chips.push(`💳 CB ${euro(pendingDeferred())}`);
 const charge=nextUnpaidCharge();if(charge)chips.push(`📅 Prochain : ${charge.label}`);
 const savings=possibleSavingsEstimate();if(savings>0)chips.push(`🎯 Épargne possible ${euro(savings)}`);
 $("dailyBriefingActions").innerHTML=chips.map(c=>`<span class="briefing-chip">${esc(c)}</span>`).join("");
}
function renderEnvelopes(){
 const rows=Object.entries(data.budgets).map(([cat,limit])=>{
  const spent=data.expenses.filter(x=>x.category===cat&&monthKey(x.date)===new Date().toISOString().slice(0,7)&&operationBelongsToActive(x)).reduce((s,x)=>s+Number(x.amount||0),0);
  const pct=limit?Math.min(100,spent/limit*100):0;
  const cls=pct>=100?"danger":pct>=80?"warning":"";
  return `<div class="envelope-row"><div class="envelope-top"><div><b>${esc(cat)}</b><small>${euro(Math.max(0,limit-spent))} restants</small></div><span>${Math.round(pct)} %</span></div><div class="envelope-track"><div class="envelope-fill ${cls}" style="width:${pct}%"></div></div></div>`;
 });
 $("envelopesSummary").innerHTML=rows.join("");
}
function applyDashboardPreferences(){
 const settings=data.settings||{};
 const briefing=$("dailyBriefingText")?.closest(".daily-briefing");
 const envelopes=$("envelopesSummary")?.closest(".card");
 const yearly=$("yearForecast")?.closest(".card");
 if(briefing)briefing.style.display=settings.showBriefing===false?"none":"";
 if(envelopes)envelopes.style.display=settings.showEnvelopes===false?"none":"";
 if(yearly)yearly.style.display=settings.showYearForecast===false?"none":"";
 $("showBriefingToggle").checked=settings.showBriefing!==false;
 $("showEnvelopesToggle").checked=settings.showEnvelopes!==false;
 $("showYearForecastToggle").checked=settings.showYearForecast!==false;
}
function calculateDebtStrategy(method,extra){
 const loans=data.loans.filter(l=>Number(l.balance)>0).map(l=>({...l}));
 loans.sort((a,b)=>method==="avalanche"?Number(b.rate||0)-Number(a.rate||0):Number(a.balance)-Number(b.balance));
 let totalMonths=0,totalInterest=0;
 const details=loans.map((l,index)=>{
  let balance=Number(l.balance),months=0,interest=0;
  const monthlyRate=Number(l.rate||0)/100/12;
  const extraForThis=index===0?extra:0;
  const payment=Math.max(1,Number(l.payment||0)+extraForThis);
  ...
  return {...l,months,interest,payment};
 });
 return {details,totalMonths,totalInterest};
}


function monthsUntil(dateStr){
 if(!dateStr)return 0;
 const now=new Date(),target=new Date(dateStr+"T12:00:00");
 return Math.max(1,(target.getFullYear()-now.getFullYear())*12+(target.getMonth()-now.getMonth()));
}
function renderProAlerts(){
 const alerts=[];
 const charge=nextUnpaidCharge();
 if(charge)alerts.push({icon:"📅",level:"",title:"Prochain prélèvement",text:`${charge.label} pour ${euro(charge.amount)} le ${charge.day||28}.`,date:`J-${Math.max(0,(charge.day||28)-new Date().getDate())}`});
 if(pendingDeferred()>0)alerts.push({icon:"💳",level:"warning",title:"Carte à débit différé",text:`${euro(pendingDeferred())} seront débités globalement le 4.`,date:"CB"});
 if(endMonth()<300)alerts.push({icon:"⚠️",level:"warning",title:"Marge faible",text:`La fin de mois est estimée à ${euro(endMonth())}.`,date:"Budget"});
 if(endMonth()<0)alerts.push({icon:"🚨",level:"danger",title:"Risque de découvert",text:"La prévision de fin de mois est négative.",date:"Urgent"});
 const overBudgets=Object.entries(data.budgets).filter(([cat,limit])=>{
  const spent=data.expenses.filter(x=>x.category===cat&&monthKey(x.date)===new Date().toISOString().slice(0,7)).reduce((s,x)=>s+x.amount,0);
  return limit>0&&spent>limit;
 });
 overBudgets.slice(0,3).forEach(([cat])=>alerts.push({icon:"📊",level:"warning",title:`Budget ${cat} dépassé`,text:"Consulte les enveloppes pour ajuster le reste du mois.",date:"Budget"}));
 $("alertCountBadge").textContent=alerts.length;
 $("proAlerts").innerHTML=alerts.length?alerts.map(a=>`<div class="pro-alert ${a.level}"><div class="pro-alert-icon">${a.icon}</div><div><b>${esc(a.title)}</b><p>${esc(a.text)}</p></div><time>${esc(a.date)}</time></div>`).join(""):'<div class="alert-item">Aucune alerte importante.</div>';
}
function cleanMerchant(label){
 return String(label||"")
  .replace(/\b(cb|paiement|prlv|sepa|facture|achat)\b/gi," ")
...
function renderMerchantAnalytics(){
 const map={};
 data.expenses.filter(operationBelongsToActive).forEach(x=>{
  const merchant=cleanMerchant(x.label);
  map[merchant]=map[merchant]||{amount:0,count:0};
  map[merchant].amount+=Number(x.amount||0);
  map[merchant].count++;
 });
 const rows=Object.entries(map).sort((a,b)=>b[1].amount-a[1].amount).slice(0,8);
 const max=Math.max(1,...rows.map(x=>x[1].amount));
 $("merchantAnalytics").innerHTML=rows.length?rows.map(([name,v])=>`<div class="merchant-row"><div class="merchant-head"><div><b>${esc(name)}</b><small>${v.count} opération${v.count>1?"s":""}</small></div><strong>${euro(v.amount)}</strong></div><div class="merchant-track"><div class="merchant-fill" style="width:${v.amount/max*100}%"></div></div></div>`).join(""):'<p class="muted">Aucune dépense à analyser.</p>';
}
function renderSubscriptionSavings(){
 const subs=detectedSubscriptions();
 const candidates=subs.filter(s=>s.avg>=5).sort((a,b)=>b.avg-a.avg).slice(0,5);
 $("subscriptionSavings").innerHTML=candidates.length?candidates.map(s=>`<div class="savings-card"><b>${esc(s.label)}</b><small>${euro(s.avg)} par mois · ${euro(s.avg*12)} par an</small><strong>Économie possible si supprimé : ${euro(s.avg*12)}/an</strong></div>`).join(""):'<p class="muted">Pas assez d’historique pour proposer des économies.</p>';
}
...
function generateProReport(){
 const t=totals(),subs=detectedSubscriptions(),merchants={};
 data.expenses.forEach(x=>{const m=cleanMerchant(x.label);merchants[m]=(merchants[m]||0)+x.amount});
...
function upcomingOperationsCount(){
 return forecastEvents().filter(x=>x.date>=new Date()).length;
}
function totalGoalSavings(){
 return data.goals.reduce((s,g)=>s+Number(g.current||0),0);
}
function renderUltimateSnapshot(){
 $("ultimateAvailable").textContent=euro(totals().available);
 $("ultimateNetWorth").textContent=euro(netWorthAfterDebt());
 $("ultimateDebt").textContent=euro(totalDebt());
 $("ultimateGoals").textContent=euro(totalGoalSavings());
 $("ultimateUpcoming").textContent=String(upcomingOperationsCount());
 $("ultimateScore").textContent=`${financialHealthScore()}/100`;
}
function renderPatrimony(){
 const assets=grossWorth(),liabilities=totalDebt(),net=netWorthAfterDebt();
 const savings=data.accounts.filter(a=>a.type==="savings").reduce((s,a)=>s+Number(a.balance||0),0);
 $("patrimonyNet").textContent=euro(net);
 $("patrimonyAssets").textContent=euro(assets);
 $("patrimonyLiabilities").textContent=euro(liabilities);
 $("patrimonySavings").textContent=euro(savings);
 $("patrimonyDebtRatio").textContent=assets?`${Math.round(liabilities/assets*100)} %`:"0 %";
 $("patrimonyTrend").textContent=net>=0?"Patrimoine positif":"Les dettes dépassent actuellement les actifs";

 const rows=forecast12Months(),values=rows.map(x=>x.balance-totalDebt()),min=Math.min(...values,0),max=Math.max(...values,1),spread=Math.max(1,max-min);
 $("patrimonyProjection").innerHTML=rows.map((x,i)=>{
  const value=values[i],height=Math.max(4,(value-min)/spread*165);
  return `<div class="patrimony-month"><b>${Math.round(value)}</b><div class="patrimony-bar ${value<0?"negative":""}" style="height:${height}px"></div><small>${esc(x.label)}</small></div>`;
 }).join("");

 const positiveAssets=data.accounts.filter(a=>Number(a.balance)>0);
 const total=Math.max(1,positiveAssets.reduce((s,a)=>s+Number(a.balance),0));
 $("assetBreakdown").innerHTML=positiveAssets.map(a=>{
  const pct=Number(a.balance)/total*100;
  return `<div class="asset-row"><div class="asset-head"><b>${esc(a.name)}</b><span>${euro(a.balance)} · ${Math.round(pct)} %</span></div><div class="asset-track"><div class="asset-fill" style="width:${pct}%"></div></div></div>`;
 }).join("")||'<p class="muted">Aucun actif positif enregistré.</p>';

 $("patrimonyGoals").innerHTML=data.goals.map(g=>{
  const pct=g.target?Math.min(100,g.current/g.target*100):0;
  return `<div class="goal"><div class="goal-head"><div><b>${esc(g.name)}</b><small>${euro(g.current)} / ${euro(g.target)}</small></div><span>${Math.round(pct)} %</span></div><div class="chart-track"><div class="chart-fill" style="width:${pct}%"></div></div></div>`;
 }).join("")||'<p class="muted">Aucun objectif.</p>';
}
function scheduledNotificationItems(){
 const items=[],days=Math.max(1,Number(data.settings.notificationDays||3)),todayDate=new Date(),todayDay=todayDate.getDate();
 if(data.settings.notifyCharges){
  data.fixedCharges.filter(x=>!x.paid&&x.day).forEach(x=>{
   let diff=Number(x.day)-todayDay;if(diff<0)diff+=31;
   if(diff<=days)items.push({icon:"📅",title:x.label,detail:`${euro(x.amount)} dans ${diff} jour${diff>1?"s":""}`,time:`J-${diff}`});
  });
 }
 if(data.settings.notifyDeferredCard&&pendingDeferred()>0){
  let diff=4-todayDay;if(diff<0)diff+=31;
  if(diff<=days)items.push({icon:"💳",title:"Débit de la carte différée",detail:`${euro(pendingDeferred())} seront débités`,time:`J-${diff}`});
 }
 if(data.settings.notifyBudgets){
  Object.entries(data.budgets).forEach(([cat,limit])=>{
   const spent=data.expenses.filter(x=>x.category===cat&&monthKey(x.date)===new Date().toISOString().slice(0,7)).reduce((s,x)=>s+x.amount,0);
   const pct=limit?spent/limit*100:0;
   if(pct>=80)items.push({icon:"📊",title:`Budget ${cat}`,detail:`${Math.round(pct)} % utilisés`,time:pct>=100?"Dépassé":"À surveiller"});
  });
 }
 if(data.settings.notifyLowBalance&&endMonth()<300)items.push({icon:"⚠️",title:"Marge de sécurité",detail:`Fin de mois estimée à ${euro(endMonth())}`,time:"Budget"});
 return items;
}
function renderNotificationCenter(){
 $("notifyCharges").checked=data.settings.notifyCharges!==false;
 $("notifyDeferredCard").checked=data.settings.notifyDeferredCard!==false;
 $("notifyBudgets").checked=data.settings.notifyBudgets!==false;
 $("notifyLowBalance").checked=data.settings.notifyLowBalance!==false;
 $("notificationDays").value=Number(data.settings.notificationDays||3);
 const items=scheduledNotificationItems();
 $("scheduledNotifications").innerHTML=items.length?items.map(x=>`<div class="notification-row"><div class="notification-icon">${x.icon}</div><div><b>${esc(x.title)}</b><small>${esc(x.detail)}</small></div><span class="notification-time">${esc(x.time)}</span></div>`).join(""):'<p class="muted">Aucune alerte programmée selon les réglages actuels.</p>';
}


function accountIcon(type,name){
 const text=(name||"").toLowerCase();
 if(type==="savings"||text.includes("livret")||text.includes("épargne"))return "🐷";
 if(type==="cash"||text.includes("espèces"))return "💶";
 if(text.includes("carte"))return "💳";
 if(text.includes("enfant")||text.includes("diego")||text.includes("maëva")||text.includes("maeva"))return "👤";
 return "🏦";
}
function renderAccountsV6(){
 $("accountsSummary").innerHTML=data.accounts.map(a=>`<div class="family-account-card"><div class="top"><div><strong>${esc(a.name)}</strong><small>${a.type==="savings"?"Épargne":a.type==="cash"?"Espèces":"Compte bancaire"}</small></div><div class="icon">${accountIcon(a.type,a.name)}</div></div><b>${euro(a.balance)}</b></div>`).join("");
}
function renderHeroV6(){
 const t=totals(),daily=Math.max(0,t.available/daysLeftInMonth()),projected=endMonth(),def=pendingDeferred();
 $("heroDaily").textContent=euro(daily);
 $("heroEndMonth").textContent=euro(projected);
 $("heroDeferred").textContent=euro(def);
 const trend=$("heroTrend");
 trend.className="trend-pill";
 if(projected<0){trend.textContent="Risque";trend.classList.add("negative")}
 else if(projected<300){trend.textContent="À surveiller";trend.classList.add("warning")}
 else{trend.textContent="Situation saine";trend.classList.add("positive")}
}
function renderSmartAgenda(){
 const events=filterAgendaEvents().slice(0,20);
 $("smartAgenda").innerHTML=events.length?events.map(e=>`<div class="agenda-item"><div class="agenda-date">${e.date.toLocaleDateString("fr-FR",{day:"2-digit",month:"2-digit"})}</div><div><b>${esc(e.label)}</b><small>${esc(e.type)}</small></div><div class="agenda-amount ${e.amount>=0?"positive":"negative"}">${e.amount>=0?"+":""}${euro(e.amount)}</div></div>`).join(""):'<p class="muted">Aucune opération sur cette période.</p>';
}
function goalIcon(name){
 const n=(name||"").toLowerCase();
 if(n.includes("vacance"))return "✈️";
 if(n.includes("voiture"))return "🚗";
 if(n.includes("noël")||n.includes("noel"))return "🎄";
 if(n.includes("travaux")||n.includes("maison"))return "🏠";
 if(n.includes("imprévu")||n.includes("imprevu"))return "🛟";
 return "🎯";
}
function renderGoalsV6(){
 $("goalsSummary").innerHTML=data.goals.length?data.goals.map(g=>{
  const pct=g.target?Math.min(100,g.current/g.target*100):0;
  return `<div class="goal-card-visual"><div class="goal-icon">${goalIcon(g.name)}</div><h3>${esc(g.name)}</h3><p>${euro(g.current)} sur ${euro(g.target)}${g.targetDate?` · cible ${fmtDate(g.targetDate)}`:""}</p><div class="goal-progress"><div class="chart-track"><div class="chart-fill" style="width:${pct}%"></div></div></div><p>${Math.round(pct)} % atteint</p></div>`;
 }).join(""):'<p class="muted">Aucun objectif d’épargne.</p>';
}


function renderInfinitySerenity(){
 const score=financialHealthScore();
 $("serenityScore").textContent=`${score}/100`;
 $("serenityFill").style.width=`${score}%`;
 let text="Situation financière stable.";
 if(score<45)text="Priorité à la sécurité et à la réduction des sorties.";
 else if(score<70)text="Situation correcte, avec quelques points à surveiller.";
 else if(score>=85)text="Très bonne marge de sérénité.";
 $("serenityText").textContent=text;
}
function filterAgendaEvents(){
 ...
 if(agendaPeriod==="today")return events.filter(e=>e.date.toDateString()===start.toDateString());
 if(agendaPeriod==="tomorrow")return events.filter(e=>e.date.toDateString()===tomorrow.toDateString());
 const maxDays=agendaPeriod==="week"?7:30;
 const end=new Date(start);end.setDate(end.getDate()+maxDays);
 return events.filter(e=>e.date>=start&&e.date<=end);
}
function renderInfinityFamilyTotals(){
 const expenses=data.expenses.reduce((s,x)=>s+Number(x.amount||0),0);
 const incomes=data.incomeTransactions.filter(x=>!x.future).reduce((s,x)=>s+Number(x.amount||0),0);
 const goals=data.goals.reduce((s,g)=>s+Number(g.current||0),0);
 $("familyTotals").innerHTML=`<div><span>Revenus enregistrés</span><b>${euro(incomes)}</b></div><div><span>Dépenses enregistrées</span><b>${euro(expenses)}</b></div><div><span>Épargne dans les objectifs</span><b>${euro(goals)}</b></div>`;
}
...
function renderGoals(){
 $("goalsSummary").innerHTML=data.goals.map(g=>{
   ...
   const possible=need===null||need<=Math.max(0,possibleSavingsEstimate());
   return `<div class="goal"><div class="goal-head"><div><b>${esc(g.name)}</b><small>${euro(g.current)} sur ${euro(g.target)}${g.targetDate?` · cible ${fmtDate(g.targetDate)}`:""}</small></div><span>${Math.round(pct)} %</span></div><div class="chart-track"><div class="chart-fill" style="width:${pct}%"></div></div>${need!==null?`<div class="goal-feasibility ${possible?"good":"warning"}"><span>Effort mensuel nécessaire</span><b>${euro(need)} / mois · ${possible?"objectif réaliste":"objectif ambitieux"}</b></div>`:""}<div class="goal-actions"><button data-goal-add="${g.id}" data-amount="10">+10 €</button><button data-goal-add="${g.id}" data-amount="50">+50 €</button><button data-goal-add="${g.id}" data-amount="100">+100 €</button></div></div>`;
 }).join("");
}
...
function renderSettings(){
 ...
 $("goalSettings").innerHTML=data.goals.map((g,i)=>`<div class="edit-row goal-edit-pro"><input data-goal-name="${i}" value="${esc(g.name)}"><input data-goal-target="${i}" type="number" step="0.01" value="${g.target}"><input data-goal-current="${i}" type="number" step="0.01" value="${g.current}"><input data-goal-date="${i}" type="date" value="${g.targetDate||""}"><button class="delete-btn" data-goal-delete="${i}">✕</button></div>`).join("");
 $("darkModeToggle").checked=data.settings.darkMode;$("showBriefingToggle").checked=data.settings.showBriefing!==false;$("showEnvelopesToggle").checked=data.settings.showEnvelopes!==false;$("showYearForecastToggle").checked=data.settings.showYearForecast!==false;$("archivesList").innerHTML=data.archives.length?data.archives.slice().reverse().map(a=>`<div class="archive-row"><div><b>${esc(a.label)}</b><small>Dépenses ${euro(a.expensesTotal)} · Solde ${euro(a.closingBalance)}</small></div><button data-archive-delete="${a.id}">✕</button></div>`).join(""):'<p class="muted">Aucune archive.</p>';
}
...
document.querySelectorAll(".period-tab").forEach(btn=>btn.addEventListener("click",()=>{
 ...
 btn.classList.add("active");agendaPeriod=btn.dataset.period;renderSmartAgenda();
}));
document.querySelectorAll(".horizon").forEach(btn=>btn.addEventListener("click",()=>{
 document.querySelectorAll(".horizon").forEach(x=>x.classList.remove("active"));
 btn.classList.add("active");forecastHorizon=Number(btn.dataset.horizon||12);renderV100Dashboard();renderPatrimony();
}));
...
$("saveExpense").addEventListener("click",async()=>{
 ...
 data.expenses.unshift({id:expenseId,amount,label:$("expenseLabel").value.trim()||"Dépense",category:$("expenseCategory").value,date:$("expenseDate").value,paymentMethod:method,debited:!deferred,accountId,profileId:activeProfileId(),attachmentId,attachmentName,note:$("expenseNote").value.trim()});
 if(!deferred)account(accountId).balance=Number((account(accountId).balance-amount).toFixed(2));
 $("expenseAmount").value="";$("expenseLabel").value="";$("expenseAttachment").value="";$("expenseNote").value="";
...
function parseCsvText(text){const lines=text.split(/\r?\n/).filter(Boolean),rows=[];for(const line of lines.slice(1)){const cells=line.split(/[;,]/).map(x=>x.replace(/^"|"$/g,"").trim()),date=cells.find(x=>/^\d{2}[\/-]\d{2}[\/-]\d{2,4}$/.test(x))||today(),nums=cells.map(x=>Number(x.replace(/\s/g,"").replace(",", "."))).filter(x=>!Number.isNaN(x)),amount=nums.length?nums[nums.length-1]:0,label=cells.find(x=>x.length>3&&!/^\d/.test(x))||"Opération";if(amount){
 ...
 rows.push({id:crypto.randomUUID(),selected:$("skipKnownDuplicates").checked?!isDuplicate:true,date:normalizedDate,label,amount:absAmount,direction,category:$("autoCategorizeImport").checked?guessCategory(label):"Autre",isDuplicate});
}}return rows}
...
document.addEventListener("click",async e=>{const iid=e.target.closest("[data-income-status]")?.dataset.incomeStatus;if(iid){const tx=data.incomeTransactions.find(x=>x.id===iid);if(tx){const received=recurringIncomeStatus(tx)==="received",acc=account(tx.accountId);tx.received=!received;tx.future=received?new Date(tx.date+"T23:59:59")>new Date():false;if(acc)acc.balance=Number((acc.balance+(received?-1:1)*Number(tx.amount||0)).toFixed(2));save()}return}const rt=e.target.closest("[data-recurring-toggle]")?.dataset.recurringToggle;if(rt){const r=data.recurringIncomes.find(x=>x.id===rt);if(r){r.enabled=r.enabled===false;save()}return}const rd=e.target.closest("[data-recurring-delete]")?.dataset.recurringDelete;if(rd){if(!confirm("Supprimer ce revenu automatique ?"))return;data.recurringIncomes=data.recurringIncomes.filter(x=>x.id!==rd);save();return}
...
$("simulatePurchase").addEventListener("click",()=>{
 ...
 const date=$("purchaseSimulationDate").value||today();
 const method=$("purchaseSimulationMethod").value;
 const category=$("purchaseSimulationCategory").value;
 const immediateImpact=method==="immediate"?amount:0;
 const projected=endMonth()-amount;
 const availableAfter=totals().available-amount;
 const safety=projected-300;
 const box=$("purchaseSimulationResult");
 box.className="simulation-result";
 if(projected<0)box.classList.add("bad");else if(projected<300)box.classList.add("warn");else box.classList.add("ok");
 box.innerHTML=`<div class="scenario-result-grid"><div><span>Disponible après achat</span><b>${euro(availableAfter)}</b></div><div><span>Fin de mois estimée</span><b>${euro(projected)}</b></div><div><span>Marge après sécurité</span><b>${euro(safety)}</b></div></div><p><b>${esc(category)}</b> · ${fmtDate(date)} · ${method==="deferred"?"CB différée":"Paiement immédiat"}. ${projected>=300?"L’achat reste compatible avec la marge de sécurité.":projected>=0?"L’achat réduit fortement la marge de sécurité.":"L’achat ferait passer la prévision en négatif."}</p>`;
});
...
$("showBriefingToggle").addEventListener("change",()=>{data.settings.showBriefing=$("showBriefingToggle").checked;save()});
$("showEnvelopesToggle").addEventListener("change",()=>{data.settings.showEnvelopes=$("showEnvelopesToggle").checked;save()});
$("showYearForecastToggle").addEventListener("change",()=>{data.settings.showYearForecast=$("showYearForecastToggle").checked;save()});
$("calculateDebtPlan").addEventListener("click",()=>{
 const method=$("debtStrategy").value,extra=Number($("extraDebtPayment").value||0);
...
$("saveNotificationSettings").addEventListener("click",()=>{
 data.settings.notifyCharges=$("notifyCharges").checked;
 data.settings.notifyDeferredCard=$("notifyDeferredCard").checked;
 data.settings.notifyBudgets=$("notifyBudgets").checked;
 data.settings.notifyLowBalance=$("notifyLowBalance").checked;
 data.settings.notificationDays=Math.max(1,Math.min(30,Number($("notificationDays").value||3)));
 save();
 alert("Alertes locales enregistrées.");
});
$("exportPatrimonyReport").addEventListener("click",()=>{
 const rows=data.accounts.map(a=>`<tr><td>${esc(a.name)}</td><td>${esc(a.type)}</td><td>${euro(a.balance)}</td></tr>`).join("");
 downloadHtml("rapport_patrimonial_ultimate.html",reportHtml("Rapport patrimonial Ultimate",[
  `<div class="section"><h2>Synthèse</h2><p>Actifs : ${euro(grossWorth())}</p><p>Dettes : ${euro(totalDebt())}</p><p>Patrimoine net : ${euro(netWorthAfterDebt())}</p></div>`,
  `<div class="section"><h2>Comptes</h2><table><tr><th>Compte</th><th>Type</th><th>Solde</th></tr>${rows}</table></div>`
 ]));
});
$("exportUltimateDashboard").addEventListener("click",()=>{
 const alerts=scheduledNotificationItems();
 downloadHtml("tableau_de_bord_infinity.html",reportHtml("Tableau de bord Infinity",[
  `<div class="section"><h2>Décision</h2><p>Disponible : ${euro(totals().available)}</p><p>Fin de mois : ${euro(endMonth())}</p><p>Score : ${financialHealthScore()}/100</p></div>`,
  `<div class="section"><h2>Patrimoine</h2><p>Actifs : ${euro(grossWorth())}</p><p>Dettes : ${euro(totalDebt())}</p><p>Net : ${euro(netWorthAfterDebt())}</p></div>`,
  `<div class="section"><h2>Alertes</h2>${alerts.map(a=>`<p><b>${esc(a.title)}</b> — ${esc(a.detail)}</p>`).join("")||"<p>Aucune alerte.</p>"}</div>`
 ]));
});
...
$("expenseDate").value=today();$("futureIncomeDate").value=today();$("purchaseSimulationDate").value=today();save();
